...
def calculate_drop_coordinate():
    ...
    for z in range(150, 0, -10):
        time_array.append((-vertical_speed - sqrt(vertical_speed**2 - 4*(altitude - z)/3.28084*g/2))/g)
        altitude_array.append(z)

    # wind speed
    wind_array = []
    for z in altitude_array:
        wind_array.append(wind_gradient(ground_speed*cos(radians(ground_direction)), ground_altitude, z, alpha_y))

    drop_coordinate = target_coordinate - displacement
    drop_coordinate.y -= trapezoidal(time_array, wind_array)*cos(wind_direction)
